  ...
  if(balls.length > 1) {
    for(var i = 0; i < balls.length; i = i + 1) {
      for(var j = i + 1; j < balls.length; j = j + 1) {
        ...
        var dist = balls[i].distance(balls[j]);
...
function Ball(Bigness, Hue, X, Y, vX, vY) {
  ...
  this.reverse = function() {
    ...
    temp.mult(this.bigness);
    this.position.add(temp);
  };


  this.distance = function(b2) {
    var dx = this.position.x - b2.position.x;
    var dy = this.position.y - b2.position.y;
